...
class Result
{
    ...
    public static string organizingContainers(List<List<int>> container)
    {
        ...
        long[] containerSum = new long[n];  // Total balls in each container
        long[] typeSum = new long[n];       // Total balls of each type

        // Calculate sums for containers and types
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < n; j++)
            {
