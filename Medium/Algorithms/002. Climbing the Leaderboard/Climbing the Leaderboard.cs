...
class Result
{
    ...
    public static List<int> climbingLeaderboard(List<int> ranked, List<int> player)
    {
        ...
        int index = distinctRanks.Count - 1;

        // For each player's score
        foreach (int score in player)
        {
            // Move up the leaderboard as long as player's score >= current rank
            while (index >= 0 && score >= distinctRanks[index])
            {
                index--;
            }
            result.Add(index + 2); // +2 because index is 0-based and we passed one rank
        }
