        ...
        for (int i = 0; i < H; i++)
        {
            for (int j = 0; j < W; j++)
            {
                if (A[i][j] > 0)
                {
                    ...
                    int front = i == 0 ? A[i][j] : Math.Max(A[i][j] - A[i - 1][j], 0);
                    int back = i == H - 1 ? A[i][j] : Math.Max(A[i][j] - A[i + 1][j], 0);
                    int left = j == 0 ? A[i][j] : Math.Max(A[i][j] - A[i][j - 1], 0);
                    int right = j == W - 1 ? A[i][j] : Math.Max(A[i][j] - A[i][j + 1], 0);
